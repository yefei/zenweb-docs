...
```ts
import { Get, Post, Context, QueryHelper, $query } from 'zenweb';
...
```ts
import { Get, Post, QueryHelper } from 'zenweb';
...
```ts
import { Get, Context, ParamHelper } from 'zenweb';
...
```ts
import { Post, BodyHelper } from 'zenweb';
...
```ts
import { Post, ObjectBody } from 'zenweb';
...
启用全局应用实例后，可以在任意位置直接使用 `$body` 系列快捷方式，无需通过依赖注入参数：

```ts title="src/controller/user.ts"
import { Post, $body, $query } from 'zenweb';

export class UserController {
  @Post()
  async create() {
    // $body 等同于注入 BodyHelper
    const data = await $body.get({
      name: '!trim',
      age: 'int',
    });

    // $getObjectBody 取得原始对象
    // const raw = $getObjectBody();

    // $getTextBody 取得文本内容
    // const text = $getTextBody();

    // $getRawBody 取得原始 Buffer
...
在 Service 层中使用全局快捷方式更加方便，无需传递 Context 参数：

```ts title="src/service/user.ts"
import { Injectable } from '@zenweb/inject';
import { $body, $log } from 'zenweb';
...
export class UserService {
  async create() {
    const data = await $body.get({
      name: '!trim',
      email: { type: '!string', validate: { email: true } },
    });
    $log.info('创建用户: %s', data.name);
    return data;
  }
}
```
...
```ts
import { Post, RawBody } from 'zenweb';
...
import { Get, Context, QueryHelper } from "zenweb";
...
## 自定义 Helper 快捷方式 [v4.0+]

除了内置的 `$query`、`$param`，你还可以通过 `$helperBase()` 工厂方法为任意 `HelperBase` 子类创建全局快捷方式：

```ts title="src/helper.ts"
import { $helperBase } from 'zenweb';
import { UploadHelper } from '@zenweb/upload';

// 为 UploadHelper 创建全局快捷方式
export const $upload = $helperBase(UploadHelper);
```
...
export class UploadController {
  @Post()
  async upload() {
    // 直接使用自定义的 $upload 快捷方式
    const fields = $upload.get({ title: 'trim' });
    const file = $upload.file('avatar');
    return { fields, file };
  }
}
```

`$helperBase()` 返回的对象拥有与原始 Helper 相同的方法（`get()`、`page()` 等），在请求上下文外调用会抛出错误。
